...
const random_digits_1 = require("../src/random-digits");
describe("secure random number", () => {
    it("should generate between range", async () => {
        ...
        for (let index = 0; index < min.length; index++) {
            const result = await (0, random_digits_1.secureRandomNumber)({
                min: min[index],
                max: max[index],
            });
    ...
    it("should throw error if min is greater than max", async () => {
        ...
        const min = 100000;
        const max = 99999;
        await expect((0, random_digits_1.secureRandomNumber)({ min, max })).rejects.toThrow();
    });
